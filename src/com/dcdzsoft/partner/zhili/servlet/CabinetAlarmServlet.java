 ...
 * @version 1.0
 */
@WebServlet(name = "/cabinetalarm", urlPatterns = { "/cabinetalarm" })
public class CabinetAlarmServlet extends HttpServlet {
	/**
	 * 柜体故障状态
	 */
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (apCfg.isLogRawMsg()) {
			log.info("/cabinetalarm =>request: " + postData.toString());
		}
		bufferReader.close();
		// 解析		
		try{
			...
			if (isOk) {
				try {
                    ...
                    if(RowSetUtils.rowsetNext(rset)){
                        InParamMBDeviceAlertQry inparam = new InParamMBDeviceAlertQry();
                        inparam.setTerminalNo(cabinetID);
                        inparam.setDepartmentID( RowSetUtils.getStringValue(rset, "DepartmentID"));
                        RowSet rset2 = MBWebClientAdapter.doBusiness(inparam);
                        ...
                        while (RowSetUtils.rowsetNext(rset2)) {
                            ...
                            dataJson.put("installadr",RowSetUtils.getStringValue(rset2, "Location"));
                            dataJson.put("alarmType",RowSetUtils.getStringValue(rset2, "AlertTypeName"));
                            String time = sdf.format(RowSetUtils.getTimestampValue(rset2, "LogTime"));
